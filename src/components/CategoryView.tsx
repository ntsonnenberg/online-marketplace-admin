...
export default function CategoryView({ categories }: Props) {
      ...
      ? [
          {
            ...
            render: (data: Data) => (
              <div
                className={`${
                  selectedCategory?._id === data._id &&
                  "bg-[#a57a5a] rounded-sm"
                } p-1`}
              >
                <button
  ...
  const renderedSelectedCategory = selectedCategory && (
    <div className="mx-10">
  ...
  const renderedModal = (
    <Modal close={() => setSelectedCategory(null)}>
      {renderedSelectedCategory}
    </Modal>
  );
